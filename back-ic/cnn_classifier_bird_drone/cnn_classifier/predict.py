...
def run(name_model: str):
    print(name_model)
    model = keras.models.load_model(f'cnn_classifier_bird_drone/tmp/{name_model}')

    labels = os.listdir('cnn_classifier_bird_drone/imgs')

    filenames = []
    imgs_arrs = []
    for filename in os.listdir('cnn_classifier_bird_drone/predict_imgs'):
        if filename.endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff')):
            img_path = f'cnn_classifier_bird_drone/predict_imgs/{filename}'
    ...
    clear_folder('cnn_classifier_bird_drone/predict_imgs')
    return pred